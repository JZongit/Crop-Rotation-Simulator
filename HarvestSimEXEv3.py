...
class DraggableIcon:
    ...
    def stop_move(self, event):
        ...
        nearest_slotx = min(self.canvas.slots, key=lambda x: abs(x + self.size / 2 - event.x))
        yanchor = (110, 110, 110, 110, 110, 110, 110, 110, 110, 110)
        nearest_sloty = min(yanchor, key=lambda y: abs(y + self.size / 2 - event.y))
        print(nearest_sloty)
        distancex = abs(nearest_slotx + self.size / 2 - event.x)
        distancey = abs(nearest_sloty + self.size / 2 - event.y)
        if distancex <= 30 and distancey <= 30:  # Snap threshold
            self.canvas.coords(self.icon, nearest_slotx, nearest_sloty, nearest_slotx + self.size, nearest_sloty +
                               self.size)
            ...
            offset = 1
            self.canvas.coords(self.text_shadow_n, nearest_slotx + self.size / 2,
                               nearest_sloty + self.size / 2 - offset)
            self.canvas.coords(self.text_shadow_ne, nearest_slotx + self.size / 2 + offset,
                               nearest_sloty + self.size / 2 - offset)
            self.canvas.coords(self.text_shadow_e, nearest_slotx + self.size / 2 + offset,
                               nearest_sloty + self.size / 2)
            self.canvas.coords(self.text_shadow_se, nearest_slotx + self.size / 2 + offset,
                               nearest_sloty + self.size / 2 + offset)
            self.canvas.coords(self.text_shadow_s, nearest_slotx + self.size / 2,
                               nearest_sloty + self.size / 2 + offset)
            self.canvas.coords(self.text_shadow_sw, nearest_slotx + self.size / 2 - offset,
                               nearest_sloty + self.size / 2 + offset)
            self.canvas.coords(self.text_shadow_w, nearest_slotx + self.size / 2 - offset,
                               nearest_sloty + self.size / 2)
            self.canvas.coords(self.text_shadow_nw, nearest_slotx + self.size / 2 - offset,
                               nearest_sloty + self.size / 2 - offset)
...
class Application(tk.Frame):
    ...
    def create_widgets(self):
        ...
        button_frame.place(x=150, y=330)  # Use fixed coordinates. Adjust x and y to match your layout needs.
        ...
        self.clear_button = Button(button_frame, text="Clear Tiers", command=self.clear_tier_entries)
        self.clear_button.pack(side='left', padx=10)  # Adjust location as needed
        self.resetperm_button = Button(button_frame, text="Reset Order", command=self.add_all_crops)
        self.resetperm_button.pack(side='left', padx=10)  # Adjust location as needed
